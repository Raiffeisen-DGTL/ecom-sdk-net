...
public partial class EcomTest
{
    ...
    [TestCategory("API")]
    [DataTestMethod]
    [DynamicData(nameof(DataGetOrderReceipts), DynamicDataSourceType.Method, DynamicDataDisplayName = nameof(DynamicDataDisplayName))]
    public void TestGetOrderReceipts(KeyValuePair<string, Action> pair)
    {
        pair.Value();
    }

    public static IEnumerable<object[]> DataGetOrderReceipts()
    {
        void OnRequest(FakeClient _, RequestEventArgs args)
        {
            Assert.AreEqual("GET", args.Method);
            Assert.AreEqual("https://test.ecom.raiffeisen.ru/api/fiscal/v1/orders/testOrder/receipts", args.Url);
        ...
        yield return DynamicDataSourceRow(
            "Unauthorized",
            () => Assert.ThrowsExceptionAsync<ServiceException>(() => ecom401.GetOrderReceipts<
        ...
        var response120 = new []
        {
            new Model.Receipt120.Receipt120Response
            {
                ReceiptNumber = "3000827351832",
